...
namespace BLL.Services
{
    ...
    public class MovieService : ServiceBase, IService<Movie, MovieModel>
    {
        ...
        public IQueryable<MovieModel> Query()
        {
            return _db.Movies.Include(m => m.Director).Include(m => m.MovieGenres).ThenInclude(mg => mg.Genre)
                .OrderByDescending(m => m.RealaseDate).ThenBy(m => m.Name)
        ...
        public ServiceBase Update(Movie record)
        {
            if (_db.Movies.Any(m => m.Id != record.Id && m.Name.ToLower() == record.Name.ToLower().Trim() && 
                m.RealaseDate == record.RealaseDate))
                return Error("Movie with the same name and realase date exists!");
            ...
            entity.RealaseDate = record.RealaseDate;
